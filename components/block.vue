<template>
  <div :id="props.section._id" class="block">
    ...
    <div
      class="block__info"
      :class="{ block__reversed: props.number % 2 === 0 }"
    >
      <div
        class="block__info__title"
        :class="{
          block__reversed__title: props.number % 2 === 0 && screenWidth >= 763,
        }"
      >
        <CpuIcon v-if="props.number % 2 !== 0 || screenWidth <= 763" />
        <p class="btn-navigate">
          {{ props.section[`navigate_${locale}`].toLocaleUpperCase() }}
        </p>
        <CpuIcon v-if="props.number % 2 === 0 && screenWidth >= 763" />
      </div>
